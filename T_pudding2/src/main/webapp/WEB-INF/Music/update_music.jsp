...
<form action="addmusic.ms" method="post" enctype="multipart/form-data">
	<table border="1" align="center" cellspacing="0" width="100%">
		...
		<th style="width: 20%;"><font size="5" >노래 제목</font></th>
			<td>
			<input type="hidden" value="${music.old_img }">
			<input type="hidden" value="${music.old_song }">
			<input type="hidden" value="${music.old_lyics }">
			<input type="hidden" value="${music.mno }">
		...
		<th style="width: 20%;"><font size="5" >장르</font></th>
			<td	>
			<input type="radio" name="music_ganre" value="Ballade" style="height: 30px;"  <c:if test="${music.music_ganre == 'Ballade' }"> checked </c:if>  ><font size="5"> 발라드</font>
			<input type="radio" name="music_ganre" value="pop"style="height: 30px;" <c:if test="${music.music_ganre == 'pop' }"> checked  </c:if> ><font size="5">팝송</font>
			<input type="radio" name="music_ganre" value="dance"style="height: 30px;" <c:if test="${music.music_ganre == 'dance' }">checked </c:if> ><font size="5">댄스</font>
			<input type="radio" name="music_ganre" value="hiphop"style="height: 30px;" <c:if test="${music.music_ganre == 'hiphop' }"> checked  </c:if> ><font size="5">힙합</font>
			<input type="radio" name="music_ganre" value="acoustic"style="height: 30px;"  <c:if test="${music.music_ganre == 'acoustic' }"> checked </c:if> ><font size="5">어쿠스틱</font>
		...
		<tr>
		<th style="width: 20%;" valign="middle"><font size="5" >노래 파일 선택</font></th>
		...
		<tr>
		<th valign="middle" style="width: 20%;"><font size="5" >가사 파일 선택</font></th>
			<td>
